...
int array2d_malloc_hostonly(array2d_t* a)
{
	...
	a->d = (fdmt_dtype*) malloc(size*sizeof(fdmt_dtype));
    assert(a->d != NULL);
    ( cudaMalloc((void**) &a->d_device, size*sizeof(fdmt_dtype) ));
    return size;
}
...
int array4d_copy_to_host(array4d_t* a)
{
	...
	(cudaMemcpy(a->d, a->d_device, size*sizeof(fdmt_dtype), cudaMemcpyDeviceToHost));
	return size;
}
...
int array4d_cuda_memset(array4d_t*a, char c) {
	...
	assert(size > 0);
	assert(a->d_device != NULL);
	(cudaMemset(a->d_device, c, size*sizeof(fdmt_dtype)));
	return size;
}
...
int array2d_copy_to_device(array2d_t* a)
{
	...
	(cudaMemcpy(a->d_device, a->d, size*sizeof(fdmt_dtype), cudaMemcpyHostToDevice));
	return size;
}
...
size_t array4d_zero(array4d_t* a) {
	...
	if (a->d_device != NULL) {
		(cudaMemset(a->d_device, 0, size*sizeof(fdmt_dtype)));
	}
