	...
	, const float VAlFmin, const  float VAlFmax, const int IMaxDT, fdmt_type_* piarrImgOut)
{
	...
	fnc_init_cpuT( piarrImgInp, IImgrows, IImgcols, ideltaT,  piarrOut_0);
	...
	*/
	...
	{
		fncFdmtIteration_cpuT( p0, val_dF, iInp0, iInp1
...
//#pragma omp parallel num_threads(nt) // OMP (main directive to launch several threds, threads quantity = nt)
#pragma omp parallel // OMP (by default nt = max quantity)
	{ // OMP (block begin
