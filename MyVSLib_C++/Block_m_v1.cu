...
{		
	...
	{
		long long quantDownloadingBytes = QUantTotalChunkBytes; 
		//
		if (i == (NumChunks-1))
		{
			long long idel = QUantTotalChannelBytes - QUantTotalChunkBytes / m_nchan * NumChunks;
...
//-----------------------------------------
bool CBlock::detailedChunkProcessing(FILE* rb_file
	...
	, CFragment* pFRg)
{
	...
	long long quantDownloadingBytes = 0;
	for (int i = 0; i < NUmChunk; ++i)
	{
		quantDownloadingBytes = (iremainedBytes < QUantTotalChunkBytes) ? iremainedBytes : QUantTotalChunkBytes;
		auto start = std::chrono::high_resolution_clock::now();

		size_t sz = downloadChunk(rb_file, d_parrInput0, quantDownloadingBytes);

		iremainedBytes = iremainedBytes - quantDownloadingBytes;
	}
	
	if (quantDownloadingBytes < QUantTotalChunkBytes)
	{		
		m_lenChunk = quantDownloadingBytes / (m_nchan * m_npol * m_nbits) * 8;
