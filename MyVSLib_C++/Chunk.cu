...
CChunk& CChunk::operator=(const CChunk& R)
{
	if (this == &R)
	{
		return *this;
...
//-----------------------------------------
bool CChunk::detailedChunkProcessing(
	...
	, CFragment* pFRg)
{
	...
	m_Fdmt.process_image(d_parr_fdmt_inp
		, pAuxBuff_fdmt
		, d_parr_fdmt_out
		, false);
...
//---------------------------------------------------
bool CChunk::fncChunkProcessing_gpu(cufftComplex* pcmparrRawSignalCur
	, void* pAuxBuff_fdmt
	, cufftComplex* pcarrTemp
	, cufftComplex* pcarrCD_Out
	, cufftComplex* pcarrBuff
...
long long CChunk::calcLenChunk_(CTelescopeHeader header, const int nsft
...
// d_parr_fdmt_inp[i][j] = 
//
void CChunk::calc_fdmt_inp(fdmt_type_* d_parr_fdmt_inp, cufftComplex* pcarrTemp
	, float*pAuxBuff)
{	
...
//--------------------------------------
void CChunk::set_chunkid(const int nC)
{
	m_Chunk_id = nC;
}
//--------------------------------------
void CChunk::set_blockid(const int nC)
{
	m_Block_id = nC;
}
//-------------------------------------------------------------------
...
//-------------------------------------------------------------------
void CChunk::fncCD_Multiband_gpu(cufftComplex* pcarrCD_Out, cufftComplex* pcarrffted_rowsignal
...
//-------------------------------------------
void CChunk::preparations_and_memoryAllocations(CTelescopeHeader header
	, const float pulse_length
	, const float d_max
	, const float sigma_bound
	, const int length_sum_wnd
