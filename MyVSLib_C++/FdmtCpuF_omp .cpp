	...
	, const float VAlFmin, const  float VAlFmax, const int IMaxDT, float* piarrImgOut)
{
	...
	piarrOut_0 = (float*)calloc(IImgrows * (ideltaT + 1) * IImgcols, sizeof(float));
	piarrOut_1 = (float*)calloc(IImgrows * (ideltaT + 1) * IImgcols, sizeof(float));
	// !4
	

	// 5  Initialize the arrays with zeros
	
	memset( piarrOut_0, 0, IImgrows * (ideltaT + 1) * IImgcols * sizeof(float));
	memset( piarrOut_1, 0, IImgrows * (ideltaT + 1) * IImgcols * sizeof(float));
	// !5
	
	// 6. call initialization func
	clock_t start = clock() * 1000.;
	fnc_init_cpuF( piarrImgInp, IImgrows, IImgcols, ideltaT,  piarrOut_0);
	...
	{
		fncFdmtIteration_cpuF( p0, val_dF, iInp0, iInp1
	...
	memcpy(piarrImgOut,  p0, IImgcols * IMaxDT* sizeof(float));
	start = clock();
	free( arr_val0);
	free( arr_val1);
	...
	, float* piarrOut, int& iOutPutDim0, int& iOutPutDim1)
{
	...
	memset( piarrOut, 0, iOutPutDim0 * iOutPutDim1 * IDim2 * sizeof(float));
	// !
	...
	{ // OMP (начало блока, который выполняется в нескольких потоках	
		for (int i_F = 0; i_F < IOutPutDim0; ++i_F)
		{
			...
			for (int i_dT = 0; i_dT < (1 + iarr_deltaTLocal[i_F]); ++i_dT)
			{
				...
				memcpy(&piarrOut[numRowOutputMtrxBegin0], &piarrInp[numRowInputMtrxBegin0], IDim2 * sizeof(float));
	...
	, const int IDeltaT, float* piarrOut)
{
	...
	memset( piarrOut, 0, IImgrows * IImgcols * (IDeltaT + 1) * sizeof(float));
    #pragma omp parallel // OMP (Если не указывать количество потоков nt, то по умолчанию будет использовано максимальное количество потоков)
	{ // OMP (начало блока, который выполняется в нескольких потоках
	...
	{ // OMP (начало блока, который выполняется в нескольких потоках
		for (int i_dT = 1; i_dT < (IDeltaT + 1); ++i_dT)
			...
			{
				float* result = &piarrOut[iF * (IDeltaT + 1) * IImgcols + i_dT * IImgcols + i_dT];
				float* arg0 = &piarrOut[iF * (IDeltaT + 1) * IImgcols + (i_dT - 1) * IImgcols + i_dT];
				float* arg1 = &piarrImg[iF * IImgcols];
