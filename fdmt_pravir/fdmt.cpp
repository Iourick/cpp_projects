...
FDMT::FDMT(float f_min
    ...
    , size_t nsamps
    , float tsamp
    , size_t dt_max// iDtMax
    , size_t dt_step //step 
    , size_t dt_min) // from this number we will store result. before it bruf force algorithm
    : f_min(f_min),
      f_max(f_max),
      nchans(nchans),
      nsamps(nsamps),
...
#pragma omp parallel for
    ...
    for (int ichan = 0; ichan < nchans; ++ichan) {
        for (int isamp = dt_init_min; isamp < nsamps; ++isamp) {
            float sum = 0.0F;
...
#pragma omp parallel for
#endif
        for (int i_dt = 0; i_dt < dt_grid_sub_size; ++i_dt) {
