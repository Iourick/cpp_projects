...
void FDMTGPU::transfer_plan_to_device(const FDMTPlan& plan, FDMTPlanD& plan_d)
{
   ...
   // plan_d.state_shape_d.resize(niter);
    std::vector<SizeType> state_shape_flattened_s ;
       
    state_shape_flattened_s.reserve(niter * 5);
    for (const auto& array : plan.state_shape)
    {        
        state_shape_flattened_s.insert(state_shape_flattened_s.end(), array.begin(), array.end());
    }
    plan_d.state_shape_d.resize(state_shape_flattened_s.size());

    std::vector<int> state_shape_flattened(state_shape_flattened_s.size());
    std::copy(state_shape_flattened_s.begin(), state_shape_flattened_s.end(), state_shape_flattened.begin());
    ...
    plan_d.lenof_innerVects_coords_cumsum.resize(h_lenof_innerVects_coords_cumsum_s.size());
    std::vector<int> h_lenof_innerVects_coords_cumsum(h_lenof_innerVects_coords_cumsum_s.size());
    std::copy(h_lenof_innerVects_coords_cumsum_s.begin(), h_lenof_innerVects_coords_cumsum_s.end(), h_lenof_innerVects_coords_cumsum.begin());  
    ...
    std::vector<int>h_lenof_innerVects_coords_to_copy_cumsum(h_lenof_innerVects_coords_to_copy_cumsum_s.size());

    std::copy(h_lenof_innerVects_coords_to_copy_cumsum_s.begin(), h_lenof_innerVects_coords_to_copy_cumsum_s.end(), h_lenof_innerVects_coords_to_copy_cumsum.begin());
    ...
    plan_d.len_mappings_cumsum.resize(h_len_mappings_cumsum_s.size());

    std::vector< SizeType>h_len_mappings_cumsum(h_len_mappings_cumsum_s.size());

    std::copy(h_len_mappings_cumsum_s.begin(), h_len_mappings_cumsum_s.end(), h_len_mappings_cumsum.begin());
    ...
    plan_d.len_mappings_to_copy_cumsum.resize(h_len_mappings_to_copy_cumsum_s.size());

    std::vector<int>h_len_mappings_to_copy_cumsum(h_len_mappings_to_copy_cumsum_s.size());
    std::copy(h_len_mappings_to_copy_cumsum_s.begin(), h_len_mappings_to_copy_cumsum_s.end(), h_len_mappings_to_copy_cumsum.begin());
    ...
    plan_d.len_state_sub_idx_cumsum.resize(h_len_state_sub_idx_cumsum_s.size());

    std::vector<int>   h_len_state_sub_idx_cumsum(h_len_state_sub_idx_cumsum_s.size());
    std::copy(h_len_state_sub_idx_cumsum_s.begin(), h_len_state_sub_idx_cumsum_s.end(), h_len_state_sub_idx_cumsum.begin());
    ...
    SizeType dt_grid_sub_min = plan.dt_grid[0][353][0];
    SizeType dt_grid_sub_min1 = dt_grid_flattened_s[pos_gridInnerVects_h_s[pos_gridSubVects_h_s[0] + 353]];
    ...
    std::copy(pos_gridSubVects_h_s.begin(), pos_gridSubVects_h_s.end(), pos_gridSubVects_h.begin());       
...
void  FDMTGPU::execute(const float* waterfall, size_t waterfall_size, float* dmt,  size_t dmt_size)
{
   ...
   // int* ppos_gridSubVects = thrust::raw_pointer_cast(m_fdmt_plan_d.pos_gridSubVects.data());
    int* ppos_gridInnerVects = thrust::raw_pointer_cast(m_fdmt_plan_d.pos_gridInnerVects.data());  //+ 
    
    int* pstate_sub_idx_cur = &pstate_sub_idx[m_fdmt_plan_d.len_state_sub_idx_cumsum[0]]; //+

    int* ppos_gridInnerVects_cur = &ppos_gridInnerVects[m_fdmt_plan_d.pos_gridSubVects[0]]; //+
  ...
  /*  int lenarr4 = plan.state_shape[0][3] * plan.state_shape[0][4];
    ...
    {       
       ...
       // const auto& state_sub_idx_prev = plan.state_sub_idx[i_iter - 1];
        ...
        int* pcoords_cur = &pcoords[m_fdmt_plan_d.lenof_innerVects_coords_cumsum[i_iter]];
            
        int* pmappings_cur = &pmappings[m_fdmt_plan_d.len_mappings_cumsum[i_iter]];

        int* pcoords_copy_cur = &pcoords_to_copy[m_fdmt_plan_d.lenof_innerVects_coords_to_copy_cumsum[i_iter]];

        int* pmappings_copy_cur = &pmappings_to_copy[m_fdmt_plan_d.len_mappings_to_copy_cumsum[i_iter]];

        int* pstate_sub_idx_cur = &pstate_sub_idx[m_fdmt_plan_d.len_state_sub_idx_cumsum[i_iter]];
        int* pstate_sub_idx_prev  = &pstate_sub_idx[m_fdmt_plan_d.len_state_sub_idx_cumsum[i_iter - 1]];
...
)
{
    ...
    if (i_coord < coords_cur_size)
    {
        ...
        if ((i_coord == 10)&&(isamp == 0))
        {
           printf("i_sub = %i,  i_dt = %i, i_sub_tail = %i, i_dt_tail = %i , i_sub_head = %i, i_dt_head = %i, offset = %i\n", i_sub, i_dt, i_sub_tail, i_dt_tail, i_sub_head, i_dt_head, offset);
           printf("state_sub_idx = %i, state_sub_idx_tail = %i, state_sub_idx_head = %i \n ", state_sub_idx, state_sub_idx_tail, state_sub_idx_head);
